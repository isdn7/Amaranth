...
# UI: 상단 대기석
# ---------------------------------------------------------
st.title("🧲 시간표 와이드 자석 보드 (90인 전체 모드)")
...
if st.session_state.buffer:
    ...
    for i, chip in enumerate(st.session_state.buffer):
        with buf_cols[i]:
            ...
            style_label = "✅ 선택됨" if is_selected else "선택"
...
with st.sidebar:
    st.header("⚙️ 설정")
    # 표시할 교사 수를 조절할 수 있게 함
    display_count = st.slider("한 번에 볼 교사 수", 1, 90, 20)
    st.write("---")
    if st.button("💾 결과 엑셀 저장"):
...
# UI: 메인 시간표 보드
# ---------------------------------------------------------
search_name = st.text_input("교사 검색", "")

# [핵심 수정] 검색어가 있으면 검색결과 전체를 보여주고, 없으면 설정한 숫자만큼 보여줌
if search_name:
    view_df = df[df['교사명'].str.contains(search_name)]
else:
    view_df = df.head(display_count)
    ...
    for i, slot in enumerate(df.columns[1:]):
        ...
        if r_cols[i+1].button(btn_label, key=f"cell_{idx}_{slot}", use_container_width=True):
            ...
            # 빼기 로직
                ...
                st.session_state.buffer.append({"teacher": t_name, "lesson": content, "orig_slot": slot})
                df.at[idx, slot] = ""
                st.rerun()
